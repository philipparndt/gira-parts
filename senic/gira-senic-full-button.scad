...
module clip(width=2, length=5.9) {
    ...
    difference() {
        ...
        union() {
            ...
            translate([2,length/2 - 2/2,-5])
                cube([2,2,10], center=false);
        }
    }
}

module clips(width=2, length=5.9, distance=38) { // 40 (37.5)
    d = distance + width;
    translate([-d/2 + width/2,-length/2,0]) {
